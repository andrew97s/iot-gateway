<template>
  <div class="app-container">
    ...
    <el-row class="mb8" :gutter="10">
      <el-col :span="1.5">
        <el-button icon="Plus" plain type="primary" v-hasPermi="['system:menu:add']" @click="handleAdd">新增</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table
      ...
      v-if="refreshTable"
    >
      <el-table-column label="菜单名称" prop="menuName" :show-overflow-tooltip="true" width="160"></el-table-column>
      ...
      <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="210">
        <template #default="scope">
          <el-button icon="Edit" link type="primary" v-hasPermi="['system:menu:edit']" @click="handleUpdate(scope.row)">修改</el-button>
          <el-button icon="Plus" link type="primary" v-hasPermi="['system:menu:add']" @click="handleAdd(scope.row)">新增</el-button>
          <el-button icon="Delete" link type="primary" v-hasPermi="['system:menu:remove']" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <!-- 添加或修改菜单对话框 -->
    <el-dialog v-model="open" append-to-body :title="title" width="680px">
              ...
              <el-popover placement="bottom-start" trigger="click" :width="540">
                <template #reference>
                  <el-input v-model="form.icon" placeholder="点击选择图标" readonly @blur="showSelectIcon">
                    <template #prefix>
                      <svg-icon class="el-input__icon" :icon-class="form.icon" style="height: 32px; width: 16px" v-if="form.icon" />
                      <el-icon style="height: 32px; width: 16px" v-else><search /></el-icon>
                    </template>
                  </el-input>
                </template>
                <icon-select :active-icon="form.icon" ref="iconSelectRef" @selected="selected" />
              </el-popover>
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12" v-if="form.menuType != 'F'">
            <el-form-item>
              ...
              <el-radio-group v-model="form.visible">
                <el-radio v-for="dict in sys_show_hide" :label="dict.value" :key="dict.value">{{ dict.label }}</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
import IconSelect from '@/components/system/IconSelect/index.vue'
