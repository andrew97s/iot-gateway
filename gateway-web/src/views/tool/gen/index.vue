<template>
  <div class="app-container">
    ...
    <el-row class="mb8" :gutter="10">
      <el-col :span="1.5">
        <el-button icon="Download" plain type="primary" v-hasPermi="['tool:gen:code']" @click="handleGenTable">生成</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button icon="Upload" plain type="info" v-hasPermi="['tool:gen:import']" @click="openImportTable">导入</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button :disabled="multiple" icon="Delete" plain type="danger" v-hasPermi="['tool:gen:remove']" @click="handleDelete">删除</el-button>
      </el-col>
      ...
      <el-table-column align="center" class-name="small-padding fixed-width" label="操作" width="330">
        <template #default="scope">
          <el-tooltip content="预览" placement="top">
            <el-button icon="View" link type="primary" v-hasPermi="['tool:gen:preview']" @click="handlePreview(scope.row)"></el-button>
          </el-tooltip>
          <el-tooltip content="编辑" placement="top">
            <el-button icon="Edit" link type="primary" v-hasPermi="['tool:gen:edit']" @click="handleEditTable(scope.row)"></el-button>
          </el-tooltip>
          <el-tooltip content="删除" placement="top">
            <el-button icon="Delete" link type="primary" v-hasPermi="['tool:gen:remove']" @click="handleDelete(scope.row)"></el-button>
          </el-tooltip>
          <el-tooltip content="同步" placement="top">
            <el-button icon="Refresh" link type="primary" v-hasPermi="['tool:gen:edit']" @click="handleSynchDb(scope.row)"></el-button>
          </el-tooltip>
          <el-tooltip content="生成代码" placement="top">
            <el-button icon="Download" link type="primary" v-hasPermi="['tool:gen:code']" @click="handleGenTable(scope.row)"></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="preview.open" append-to-body class="scrollbar" :title="preview.title" top="5vh" width="80%">
      <el-tabs v-model="preview.activeName">
        <el-tab-pane
          ...
          :key="value"
        >
          <el-link icon="DocumentCopy" style="float: right" :underline="false" v-copyText="value" v-copyText:callback="copyTextSuccess">
            &nbsp;复制
          </el-link>
          <pre>{{ value }}</pre>
        </el-tab-pane>
